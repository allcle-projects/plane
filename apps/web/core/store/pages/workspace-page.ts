...
export class WorkspacePage extends BasePage implements TWorkspacePage {
  constructor(store: RootStore, page: TPage) {
    ...
    super(store, page, {
      ...
      duplicate: async () => {
        // Duplication is not supported for workspace/wiki pages (no backend endpoint).
        throw new Error("Duplicating wiki pages is not supported.");
      },
    });
    makeObservable(this, {
      // computed
  ...
  /**
   * @description workspace/wiki pages cannot be duplicated (no backend endpoint)
   */
  ...
  /**
   * @description workspace/wiki pages are not tied to a project and cannot be moved
   */
  ...
  get isContentEditable() {
    ...
    return (
      !isArchived &&
      !isLocked &&
      (isOwner || (isPublic && !!workspaceRole && workspaceRole >= EUserPermissions.MEMBER))
    );
  }
